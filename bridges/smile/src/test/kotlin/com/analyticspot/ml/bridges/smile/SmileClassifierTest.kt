...
class SmileClassifierTest {
    ...
    fun testSimpleClassifier() {
        ...
        val wrongDs = dg.createTrainingSource("A2", 0.2, true, "T3")
    ...
    data class DataAndTarget(val data: DataSet, val targetId: CategoricalFeatureId)
    ...
    private fun generateDtData(numRows: Long, seed: Long): DataAndTarget {
        ...
        val targetId = CategoricalFeatureId("Target", false, setOf("T1", "T2", "T3"))

        val data = DataSet.build {
            addColumn(CategoricalFeatureId("A", false, possibleAValues.toSet()), a)
            addColumn(NumericalFeatureId("B", false), b)
            addColumn(BooleanFeatureId("C", false), c)
            addColumn(targetId, target)
        }
